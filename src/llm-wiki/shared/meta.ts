...
const META_KEYS = ['title', 'discipline', 'platform', 'source', 'author', 'year', 'verified'] as const;
type MetaKey = (typeof META_KEYS)[number];
const META_SCAN_LINES = 20;

export type Meta = Record<MetaKey, string>;

export async function extractMeta(filePath: string): Promise<Meta> {
  const empty = Object.fromEntries(META_KEYS.map((k) => [k, ''])) as Meta;
