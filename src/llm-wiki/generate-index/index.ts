...
import {
  ...
  CATEGORY_ORDER, categorize, type Meta,
...
type DisciplineGroups = Record<string, Record<string, Page[]>>;

function groupByDiscipline(allPages: AllPages): DisciplineGroups {
...
interface CategoryData {
  disciplines: Record<string, Record<string, Page[]>>;
}

type Organized = Record<string, CategoryData>;

function organizeByCategory(disciplineGroups: DisciplineGroups): Organized {
...
function renderSection(types: Record<string, Page[]>): string[] {
  const lines: string[] = [];
...
function renderIndex({ allPages, totalPages, rawCount, organized }: RenderArgs): string {
  const lines: string[] = [];
