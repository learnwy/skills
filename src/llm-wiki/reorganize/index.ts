...
import { WIKI_DIR } from '../shared/index.js';
...
function classifyDomain(discipline: string): string {
...
const FILENAME_MAP: FilenameRule[] = [
...
const BOOK_DOMAIN_MAP: Record<string, string> = {
...
async function classifyBySourceLinks(filePath: string): Promise<string | null> {
  try {
    const content = await readFile(filePath, 'utf-8');
...
async function run(): Promise<void> {
  ...
  for (const file of files) {
    ...
    let domain: string | null = discipline ? classifyDomain(discipline) : null;
