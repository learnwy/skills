...
import { WIKI_DIR, PAGE_DIRS } from '../shared/index.js';
...
interface StalePage extends PageInfo {
  ageDays: number;
  threshold: number;
}

interface AgedPage extends PageInfo {
  ageDays: number;
}
...
async function extractPageMeta(filePath: string): Promise<PageMeta> {
  const empty: PageMeta = { title: '', discipline: '', ingested: '', lastVerified: '', verified: '' };
  try {
...
const isFastMoving = (discipline: string): boolean => {
  const d = discipline.toLowerCase();
  return FAST_MOVING_DOMAINS.some((domain) => d.includes(domain));
};

async function scanDir(baseDir: string, subdir: string): Promise<PageInfo[]> {
...
async function run(): Promise<void> {
  ...
  for (const subdir of PAGE_DIRS) {
    ...
    for (const page of pages) {
      ...
      if (ageDays > threshold) stale.push({ ...page, ageDays, threshold });
      if (page.verified === 'no') unverified.push({ ...page, ageDays });
    }
  }
