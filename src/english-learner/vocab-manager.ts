...
interface BatchWordItem extends SaveWordOpts {
  word?: string;
}
...
function main(): void {
  ...
  if (cmd === 'get_word' && args.length >= 2) {
    ...
    const examples = args[4] ? (JSON.parse(args[4]) as string[]) : [];
    ...
    const words = JSON.parse(args[1]) as string[];
    console.log(JSON.stringify(batchGetWords(words), null, 2));
  } else if (cmd === 'batch_save' && args.length >= 2) {
    const wordsData = JSON.parse(args[1]) as BatchWordItem[];
