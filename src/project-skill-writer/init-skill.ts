...
interface SkillTypePattern {
  keywords: string[];
  type: string;
}

const SKILL_TYPE_PATTERNS: SkillTypePattern[] = [
...
const TRIGGER_PREFIXES: Record<string, string[]> = {
...
function extractKeywords(text: string, maxCount = 5): string[] {
...
function slugify(words: string[], maxParts = 3): string {
  return words.slice(0, maxParts).join('-');
}

type Mapping = Record<string, string>;

function inferFromProblem(problem: string): Mapping {
  ...
  return {
    ...
    COL1: 'Input', COL2: 'Action', COL3: 'Output',
    VAL1: 'User request', VAL2: 'Skill ' + skillType, VAL3: 'Structured result',
    ISSUE1: 'Missing input', SOLUTION1: 'Request required fields',
    ISSUE2: 'Unsupported scope', SOLUTION2: 'Explain supported boundaries',
    REF_1: 'Reference', REF_1_FILE: 'reference.md', REF_1_DESC: 'Domain notes',
    REF_2: 'Template', REF_2_FILE: 'template.md', REF_2_DESC: 'Output template',
  };
}

function defaults(skillName: string, summary: string): Mapping {
  ...
  return {
    ...
    STEP_4: 'Verify output',
    COL1: 'Input', COL2: 'Action', COL3: 'Output',
    VAL1: 'User request', VAL2: 'Skill workflow', VAL3: 'Structured result',
    ISSUE1: 'Missing input', SOLUTION1: 'Request required fields',
    ISSUE2: 'Unsupported scope', SOLUTION2: 'Explain supported boundaries',
    REF_1: 'Reference', REF_1_FILE: 'reference.md', REF_1_DESC: 'Domain notes',
    REF_2: 'Template', REF_2_FILE: 'template.md', REF_2_DESC: 'Output template',
...
function render(template: string, mapping: Mapping): string {
  let out = template;
