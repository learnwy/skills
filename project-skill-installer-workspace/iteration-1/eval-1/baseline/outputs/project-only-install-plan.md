...
- 执行原则：先完成可执行前置检查，再委托 `find-skills` 做发现与安装动作编排。
...
- 本次产物目录（项目内）：`/Users/wangyang.learnwy/learnwy/learnwy/skills/project-skill-installer-workspace/iteration-1/eval-1/baseline/outputs/`
...
选择理由：`skills/` 是仓库内现有的项目级技能目录，满足 project-only 约束且避免全局写入风险。

## 3) 前置检查（Prerequisites Gate）

### 检查项 A：`find-skills` 全局可用

1. 先检查 `find-skills` 是否可调用。
2. 若不可用，阻断安装流程并提示先安装：
...
1. 安装目标必须位于 `/Users/wangyang.learnwy/learnwy/learnwy/skills` 内。
2. 若目标路径解析到 `~/.trae/skills` 或 `~/.trae-cn/skills`，立即拒绝并改回项目路径。

## 4) 委托 `find-skills` 的执行步骤

1. 以“iOS 崩溃分析”为查询意图调用 `find-skills`，并附带 project-only 约束。
2. 让 `find-skills` 仅返回可安装到项目目录的候选 skill。
3. 选定候选后执行安装动作：
   - 创建目录：`skills/ios-crash-analysis/`
   - 写入核心文件：`skills/ios-crash-analysis/SKILL.md`
   - 按需补充 `references/`、`scripts/` 等子目录
4. 输出安装摘要：创建文件清单、目标路径、约束校验结果。

## 5) 失败分支与回退策略

- `find-skills` 不可用：阻断并提示先执行 `npx skills add find-skills -g -y`。
- 候选仅支持全局安装：拒绝该候选并继续检索项目级候选。
- 路径冲突或不可写：切换到项目内可写目录 `skills/`，不触发全局回退。

## 6) 执行摘要

- 已明确项目路径证据与项目内安装目标，目标位于 `skills/ios-crash-analysis/`。
- 已纳入 `find-skills` 前置检查与缺失时阻断策略。
- 已定义从委托发现到项目内安装与失败回退的完整可执行流程，并全程保持 project-only 约束。
